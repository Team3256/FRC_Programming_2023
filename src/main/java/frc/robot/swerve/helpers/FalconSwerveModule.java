...
public class FalconSwerveModule extends SubsystemBase {
    ...
    while (Timer.getFPGATimestamp() - start < 10.0) {
      try {
        ...
        if (Timer.getFPGATimestamp() - start >= 10.0) {
          System.out.println(
              "TALON CONFIGURATION TIMED OUT: PORTS " + driveId + " AND " + steeringId);
        }
      }
    }
  ...
  public double getUnclampedSteerAngleRadians() {
    return (mSteeringMotor.getSelectedSensorPosition() * kSteerPositionCoefficient)
        - mTalonOffset.getRadians();
  }
  ...
  public void setWithVelocityShortestPath(double driveVelocity, Rotation2d steerAngle) {
    ...
    mDriveMotor.set(
        ...
        (flip ? -driveVelocity : driveVelocity) / kDriveVelocityCoefficient);
  }
