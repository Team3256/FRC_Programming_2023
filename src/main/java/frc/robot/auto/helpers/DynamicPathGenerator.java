...
public class DynamicPathGenerator {
  ...
  public PathPlannerTrajectory computeTrajectory() {
    PathPoint currentPoint = new PathPoint(startPose.getTranslation(), new Rotation2d(), startPose.getRotation());

    double[][] updatedGraph = addStartAndEndNodes(pathAdjacencyGraph, startPose.getTranslation(),
        goalPose.getTranslation());

    // The last two nodes are the start and goal nodes in the updatedGraph
    ArrayList<Integer> pathIndexes = AStar.findPath(updatedGraph, heuristic, updatedGraph.length - 2,
        updatedGraph.length - 1);

    List<PathPoint> waypoints = new ArrayList<>();
    waypoints.add(new PathPoint(startPose.getTranslation(), new Rotation2d(), startPose.getRotation()));
    ...
    waypoints.add(new PathPoint(goalPose.getTranslation(), new Rotation2d(), goalPose.getRotation()));
