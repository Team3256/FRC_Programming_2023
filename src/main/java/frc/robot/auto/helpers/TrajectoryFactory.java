...
public class TrajectoryFactory {
  ...
  public Command createPathPlannerCommand(String path, Pose2d startPose) {
    PathPlannerTrajectory trajectory =
        PathPlanner.loadPath(
            path, kMaxSpeedMetersPerSecond, kMaxAccelerationMetersPerSecondSquared);
  ...
  public Command createPathPlannerCommand(
      String path, AutoCommandRunner autoCommandRunner, boolean firstSegment) {
    PathPlannerTrajectory trajectory =
        PathPlanner.loadPath(
            path, kMaxSpeedMetersPerSecond, kMaxAccelerationMetersPerSecondSquared);
    PPTrajectoryFollowCommand command = getCommand(trajectory);
  ...
  public Command createCommand(double start, double end, Direction direction) {
    ...
    Trajectory trajectory =
        TrajectoryGenerator.generateTrajectory(waypoints, getDefaultTrajectoryConfig());
  ...
  public Command createCommand(
      ...
      double end,
      Direction direction,
      ThetaSupplier thetaSupplier,
      boolean reversed) {
    List<Pose2d> waypoints = createStraightWaypoints(start, end, direction);

    Trajectory trajectory =
        TrajectoryGenerator.generateTrajectory(waypoints, getDefaultTrajectoryConfig(reversed));
  ...
  public Command createCommand(
      double start, double end, Direction direction, ThetaSupplier thetaSupplier) {
    List<Pose2d> waypoints = createStraightWaypoints(start, end, direction);
  ...
  private Command getCommand(
      Trajectory trajectory, ThetaSupplier uniformThetaSupplier, Pose2d startPose) {
    ProfiledPIDController thetaController =
        new ProfiledPIDController(
            P_THETA_CONTROLLER,
            I_THETA_CONTROLLER,
  ...
  private PPTrajectoryFollowCommand getCommand(
      PathPlannerTrajectory trajectory, double thetakp, double thetaki, double thetakd) {
    ProfiledPIDController thetaController =
        new ProfiledPIDController(thetakp, thetaki, thetakd, THETA_CONTROLLER_CONSTRAINTS);
    thetaController.enableContinuousInput(-2 * Math.PI, 2 * Math.PI);
  ...
  private Trajectory generateTrajectoryFromJSON(String trajectoryFile) {
    ...
    switch (fileExtension) {
      case "path": // for PathPlanner files
        ...
        trajectory =
            PathPlanner.loadPath(
                path, kMaxAngularSpeedRadiansPerSecond, kMaxAngularSpeedRadiansPerSecondSquared);
        break;
      case "json": // for wpilib PathWeaver files
        try {
          ...
          DriverStation.reportError(
              "Unable to open trajectory: " + trajectoryFile, ex.getStackTrace());
        }
        break;
      default:
